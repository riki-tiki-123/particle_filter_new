...
def run_tracking_example(
    ...
    y_max = np.max(valid_y) + 10

    for idx in dropout_indices:
        for b in range(n_balls):
            x = true_positions_over_time[idx, b, 0]
            ax.fill_betweenx(y=[0, y_max], x1=x - 0.5, x2=x + 0.5,
                             color='purple', alpha=0.3)

    for b in range(n_balls):
        mask_true = true_positions_over_time[:, b, 1] >= 0
    ...
    for b in range(n_balls):
        axs[0].plot(time_list, true_positions_over_time[:, b, 1], linestyle='--', label=f"True Y Ball {b+1}")
        est_y = [est_positions_over_time[i, b, 1] for i in range(len(time_list)) if observations[i] is not None]
        est_t = [time_list[i] for i in range(len(time_list)) if observations[i] is not None]
        axs[0].plot(est_t, est_y, marker='o', linestyle='None', label=f"Est Y Ball {b+1}")
        obs_y = [observations[i][b][1] if observations[i] is not None else np.nan for i in range(len(time_list))]
        axs[0].plot(time_list, obs_y, linestyle='-', alpha=0.4, label=f"Obs Y Ball {b+1}")

        axs[1].plot(time_list, true_positions_over_time[:, b, 0], linestyle='--', label=f"True X Ball {b+1}")
        est_x = [est_positions_over_time[i, b, 0] for i in range(len(time_list)) if observations[i] is not None]
        axs[1].plot(est_t, est_x, marker='o', linestyle='None', label=f"Est X Ball {b+1}")
        obs_x = [observations[i][b][0] if observations[i] is not None else np.nan for i in range(len(time_list))]
    ...
    for b in range(n_balls):
        ax.plot(true_positions_over_time[:, b, 0],
                ...
                time_list, linestyle='--', label=f"True Ball {b+1}")
        est_x = [est_positions_over_time[i, b, 0] for i in range(len(time_list)) if observations[i] is not None]
        est_y = [est_positions_over_time[i, b, 1] for i in range(len(time_list)) if observations[i] is not None]
        est_t = [time_list[i] for i in range(len(time_list)) if observations[i] is not None]
    ...
    ax.set_title("3D Trajectory View (X, Y, Time)")
    ax.legend()
    plt.tight_layout()
